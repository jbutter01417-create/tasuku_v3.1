...
const drawCanvas = () => {
  ...
  // Completed crystals (orbit)
  completedCrystals.forEach((crystal, i) => {
    const angle = (i / Math.max(completedCrystals.length, 1)) *
      Math.PI * 2 + timeCounter * 0.0003;
...
const resizeCanvas = () => {
  ...
  const ctx = canvas.getContext("2d");
  ctx.setTransform(scale, 0, 0, scale, 0, 0); // ←これが重要
};
...
const updateUI = () => {
  ...
  if (universe.tasks.length === 0) {
    ...
    taskList.innerHTML = universe.tasks
      .map(task => `
        ...
        <div class="subtask-list">
          ${task.subtasks.map(sub => `
            <label class="subtask-item">
              ...
              <span class="subtask-text ${sub.done ? 'done' : ''}">
                ${sub.text}
              </span>
            </label>
          `).join('')}
  ...
  universeGrid.innerHTML = state.universes.map(u => `
    <div class="universe-thumbnail ${u.id === state.activeUniverseId ? 'active' : ''}"
